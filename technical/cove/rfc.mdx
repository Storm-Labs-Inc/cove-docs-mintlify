...
<Accordion title="Details (Click to expand)">
    ...
    - Output
        - Each **Weight Proposer** calls `WeightStrategy.proposeBasketWeights(address[] baskets, uint[] weights)`
...
<Accordion title="Details (Click to expand)">
    ...
    - Output:
        - During `BasketManager.proposeRebalance()`
            - Each basket’s target weight and the protocol’s target weight is stored.
</Accordion>

3. **TokenSwap Proposer (DAO)** calculates the optimal swap pairs to reach the target amounts of each basket and calls `BasketManager.proposeTokenSwap(Trade[] internalTrades, Trade[] externalTrades)` to submit all necessary swap amounts and associated basket information. BasketManager performs minimal on-chain validation to ensure the inputs align with each basket's target weights. If more advanced validation is needed, the role can be assigned to a separate contract using [Axiom](https://www.axiom.xyz/), transforming the process from a variable to a fixed cost.
...
<Accordion title="Details (Click to expand)">
    ...
    - Checks
        - Check if the each trade’s value change with slippage is within threshold (no faulty swaps leading to value loss).
...
<Accordion title="Details (Click to expand)">
    ...
    - Outputs:
        - Submits the orders to CoWSwap API to be executed.
...
<Accordion title="Details (Click to expand)">
    ...
    - Checks:
        - If target weights are not met, enters a retry state. Returns rebalance status to "REBALANCED_PROPOSED" and starts the process of proposing token swaps again.
    - Inputs:
    ...
    - Effects:
        ...
        - Updates the basket managers internal accouting of each baskets balance of traded tokens.
        - Recalculates each basket’s pricePerShare. //< I think this should be removed
        - Advances the epoch.
</Accordion>
...
- `proposeTokenSwap`
    ...
    - Extends the current rebalance window for 15 more minutes to give time for swaps to be executed.
...
- `completeRebalance`
    - Checks if close to target weights.
...
sequenceDiagram
    ...
    CoWSwapAdapter->>CoWSwapAdapter: Decode, validate and emit events
    CoWSwapAdapter->>CoWSwapAdapter: Store orderUIDs/orderHashes
    CoWSwapAdapter-->>BasketManagerSC: end delegatecall
    deactivate CoWSwapAdapter
    deactivate BasketManagerSC

    %% Stage 3: Token Swap Intent Execution
    TokenSwapExecutor-->>BasketManagerSC: Read orderHashes
    BasketManagerSC-->>TokenSwapExecutor: Return orderHashes

    TokenSwapExecutor->>CowSwapAPI: Submit orders for execution
    activate CowSwapAPI
    CowSwapAPI-->>BasketManagerSC: ERC1271.isValidSignature(hash, bytes)
    BasketManagerSC-->>CowSwapAPI: Signature verification and swap execution
    deactivate CowSwapAPI
...
Cove will use the [ERC-7540](https://eips.ethereum.org/EIPS/eip-7540) standard for LP tokens and include permit functionality using Uniswap’s [Permit2](https://github.com/Uniswap/permit2) (EIP-2612).

### Pricing LP tokens

#### Problems

- It is difficult to know the price of each Basket’s tokens in USDT due to the portfolio aspect of Baskets, as it can be currently holding any combinations of its assets each epoch. <- maybe better wording needed

#### Solutions

- new totalAssets() method??
...
Cove uses oracles to value baskets and settle internal trades.  Cove will support pluggable oracle implementations.  The initial implementation will first remove? ->(sanity check each of the responses (e.g. stablecoins should be between $0.90 and $1.10 or ETH LSTs should be between 0.9 and 1.1 ETH)) and then validate that primary response is within some threshold of the secondary/tertiary responses.  If any checks fail, the associated baskets will be paused until resumed by the operational multisig or governance.
...
- Is below handled by the farming plugin or will it be separate functions? if so we do not have them yet***
...
- Large amount of Baskets could be created to DDOS the system. ** is this a realiztic risk with permissioned bm? (also new baskets are limited by asset + bitflag combination)
