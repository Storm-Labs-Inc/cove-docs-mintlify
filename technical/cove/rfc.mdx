    ...
    - Output
        - Each **Weight Proposer** calls `WeightStrategy.setTargetWeights(uint256 bitFlag, uint256[] weights)`
            ...
            - Piecemeal - Each rebalance proposer submits new weights for baskets of each allocation type
                ...
                2. WeightStrategyC = market cap weighted
                    1. Basket E weights
...
### Verifiable Computation with Trusted Execution Environments (TEEs)

The off-chain determination of optimal internal and external trades via linear programming introduces an element of trust. Participants in the Cove protocol rely on the TokenSwap Proposer to execute this complex computation faithfully and submit a genuinely optimized set of trades. While DAO governance provides a layer of oversight, the computational process itself remains opaque to external verifiers.

Trusted Execution Environments (TEEs) offer a compelling solution to enhance the verifiability and integrity of this critical step. A TEE, such as Intel SGX or AMD SEV, provides a hardware-isolated environment within a processor where code and data can be protected against observation or tampering by the host system, including the operating system or hypervisor.

**Benefits for Cove's Rebalancing Optimization:**

1.  **Remote Attestation & Integrity**: The linear programming solver can be executed within a TEE. Crucially, TEEs support *remote attestation*, a process where the TEE cryptographically proves to a remote party (e.g., the BasketManager contract or users) that a specific, audited piece of software (the solver) was loaded and executed correctly within the secure enclave, and that the output (the proposed trades) is the genuine result of this computation. This provides strong integrity guarantees for the optimization process, ensuring the proposed trades haven't been manipulated. [ZTEE]

2.  **Reduced Trust Assumptions**: By leveraging TEEs, the protocol can significantly reduce the trust placed in the TokenSwap Proposer's operational security. Instead, trust shifts to the well-defined security properties of the TEE hardware and the attestation mechanism. This aligns with the broader DeFi ethos of minimizing reliance on trusted intermediaries.

3.  **Confidentiality (Potential Future Enhancement)**: While the primary benefit here is verifiable execution, TEEs also offer confidentiality. If parts of the input data for the linear programming (e.g., very specific trading intentions beyond basket weights) were deemed highly sensitive in future iterations, TEEs could protect this data even from the host running the computation.

4.  **Enhanced Transparency**: While the computation inside the TEE is a "black box" to the host, the remote attestation process makes the *choice of code* and the *integrity of its execution* transparent and verifiable. This is a step towards what the Flashbots article terms "Zero Trust Execution Environments," aiming to minimize trust dependencies. [ZTEE]

**Considerations:**

Implementing TEE-based verifiable computation involves its own set of challenges, including the complexity of TEE application development, potential performance overheads (though likely acceptable for this off-chain process), and the inherent trust in the TEE hardware manufacturer and the key provisioning process. However, the substantial increase in verifiability and the reduction of trust in off-chain computation make TEEs a promising avenue for bolstering the security and trustworthiness of Cove's rebalancing mechanism.

[ZTEE] Flashbots. (2024-10-11). *Zero Trust Execution Environments*. [https://writings.flashbots.net/ZTEE](https://writings.flashbots.net/ZTEE)
...
$$ fee = totalSupply \times managementFeeBps \times \frac{timeSinceLastHarvest}{(1e4 - managementFeeBps) \times 365 days} $$
...
Cove employs oracles to value baskets and settle internal trades. The protocol is designed to support pluggable oracle implementations using [euler-price-oracle](https://github.com/euler-xyz/euler-price-oracle).

### Oracle Adapter Types

Cove defines the following oracle adapter types:
-   **ERC-4626 Oracle**: Converts between an ERC-4626 vault's shares and its underlying asset using `convertToAssets` and `convertToShares`. This adapter is intended for use when no direct oracle exists for the vault token.
-   **Chained ERC-4626 Oracle**: Extends the ERC-4626 Oracle functionality to a chain of vaults (maximum depth of 10) until a target asset is reached, performing sequential conversions along the chain.
-   **Curve EMA Oracle (Underlying)**: Reads the Exponential Moving Average (EMA)-based pricing exposed by Curve Stable-Swap-NG pools. Configuration flags determine whether the pool's price is expressed in vault shares or underlying assets.

### Anchored Oracle Behaviour

The `AnchoredOracle` implementation includes the following behaviors:
-   Automatic rescaling of the input amount if the primary quote is less than 1 WAD (i.e., `1e18`) to preserve precision.
-   Explicit `ScalingOverflow` revert on extreme input values.
-   The `maxDivergence` parameter is bounded between 0.1% and 50%; the constructor will revert if values outside this range are provided.

### Interface and Registration

-   Cove oracle adapters **must** conform to the **EIP-7726** (`IPriceOracle`) interface.
-   Oracle registration is managed via an `OracleRouter` contract, which is referenced by the `BasketManager`. Deployment scripts are responsible for the on-chain registration of each adapter. If an `AnchoredOracle` instance is used, its corresponding (primary, anchor) oracle pair must also be registered.

The primary oracle implementation utilized is `AnchoredOracle`. This smart contract implements the `IPriceOracle` interface by retrieving quotes from a primary oracle and validating them against an anchor oracle. The primary quote must be within a specified divergence range from the anchor quote before the price is returned. This mechanism provides an additional layer of security against stale or manipulated oracle prices.

For assets that cannot be priced directly in USD, a `CrossAdapter` is utilized with the base asset.
...
### Potential Griefing Attack via Pending Deposits/Redeems

When users call `requestDeposit` or `requestRedeem`, the system ensures that the controller does not have any pending or claimable deposits or redeems from the controller's `lastDepositRequestId`.
...
**Recommendation for Integrators**: When integrating the `BasketToken` into other contracts, always check for any pending or claimable tokens before requesting a deposit or redeem. This ensures that any pending deposits or redeems are resolved, preventing such griefing attacks.
