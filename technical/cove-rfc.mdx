...
<Accordion title="Details (Click to expand)">
    ...
    - Outputs:
        - Submits the orders to CoWSwap API for to be executed.
...
<Accordion title="Details (Click to expand)">
    ...
    - Checks:
        - Rejects if we are waiting for swaps to be executed.
    - Inputs:
    ...
    - Effects:
        - Saves the current token balances and the baskets’s ownership targets as actual values used by BM.
        - Recalculates each basket’s pricePerShare.
        - Advances the epoch.
</Accordion>
...
- `proposeTokenSwap`
    ...
    - Opens a 15-minute window for `executeTokenSwap`.
